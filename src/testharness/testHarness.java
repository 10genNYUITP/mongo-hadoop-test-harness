...
public class testHarness{
	...
	public static void main(String[] args) {
		...
		try {
			if (testSlaveOk) {
				Boolean ntf = new Boolean[]{null, Boolean.TRUE, Boolean.FALSE}; 
	
				ConfigFileReader cfrNew = ConfigFileReader.parse(new FileReader(fileName));
				
				Iterator i1 = cfrNew.testcases.entrySet().iterator();
				while (i1.hasNext()) {	
					Map.Entry pairs = (Map.Entry) i1.next();
					TestCase tcase = pairs.getValue();
					String tname = pairs.getKey();
					String collection = tname + "_" + crfNew.args.toArray();
